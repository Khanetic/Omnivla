...
_PED_SPEED   = 0.5              # pedestrian walking speed (m/s)
_CAR_MAX_VEL = _PED_SPEED * 1.3  # car velocity ceiling — can close the gap
...
def waypoints_to_vel(waypoints: np.ndarray, tick_hz: float = 3.0) -> Tuple[float, float]:
    ...
    lin = float(np.clip(lin, 0.0, _CAR_MAX_VEL))
    ang = float(np.clip(ang, -1.0, 1.0))

    maxv = _CAR_MAX_VEL
    maxw = 0.3
    if abs(ang) <= maxw:
        # Turn is gentle — output directly, just cap forward speed
        vx = float(np.clip(lin, 0.0, maxv))
        wz = ang
    else:
        # Turn is sharp — scale forward speed proportionally to maintain the
        # turning radius instead of capping wz and killing vx (which causes spinning)
        scale = maxw / (abs(ang) + 1e-9)
        vx    = float(np.clip(lin * scale, 0.0, maxv))
        wz    = maxw * float(np.sign(ang))
        if vx < 0.02:       # essentially stationary — no point steering
            vx, wz = 0.0, 0.0
...
def vel_to_control(vx: float, wz: float, dist_m: float,
                   min_dist: float = 2.0, max_dist: float = 7.0):
    """(vx, wz, dist) → carla.VehicleControl with distance-keeping override.

    Distance zones:
      < 1.0 m  : emergency stop — full brake
      1.0-2.0 m: proportional slowdown to zero
      2.0-7.0 m: normal following
      > 7.0 m  : slight boost to close gap
    """
    if dist_m > 0:
        if dist_m < 1.0:
            vx = 0.0
        elif dist_m < min_dist:
            vx = vx * (dist_m - 1.0) / (min_dist - 1.0)
        elif dist_m > max_dist:
            vx = min(vx * 1.1, _CAR_MAX_VEL)   # proportional boost to close gap
    throttle = float(np.clip(vx / _PED_SPEED * 0.45, 0.0, 1.0))
    # Proportional brake in emergency zone — avoids harsh jolt at 1.0 m
    brake    = float(np.clip(1.0 - dist_m, 0.0, 0.8)) if (dist_m > 0 and dist_m < 1.0) \
               else (0.3 if vx < 0.02 else 0.0)
...
def navigation_thread(
    ...
    _lost_ticks = 0
    _LOST_TIMEOUT = 5    # ticks before we stop using stale bbox (~1.7s at 3 Hz)
    ...
    while state.running:
        ...
        if not visible:
            _lost_ticks += 1
            if last_bbox is None or _lost_ticks > _LOST_TIMEOUT:
        ...
        try:
            ...
            state.write_velocity(0.0, 0.0)   # safe stop on any error
        except Exception as exc:
...
def main():
    ...
    LANG_GOAL = "follow the person ahead while keeping a safe distance"
    ...
    def cleanup(sig=None, _f=None):
        ...
        try:
            camera.stop_listening()  # close stream before destroying actor
    ...
    # _PED_SPEED is the module-level constant (0.8 m/s = natural walking pace).
    ...
    for step in range(args.max_steps):
        ...
        if not visible:
            # Don't brake when LOST — creep straight ahead (zero wz prevents
            # the car from steering hard on a stale angular command).
            if vx > 0.01:
                ctrl = vel_to_control(vx * 0.5, 0.0, 10.0)
                ctrl.throttle = max(ctrl.throttle, 0.12)
            else:
                ctrl = carla.VehicleControl(throttle=0.12, brake=0.0)
            if cur_spd > _PED_SPEED * 1.2:
                ctrl.throttle = 0.0
                ctrl.brake    = 0.2
            ...
            if ctrl.throttle < 0.05 and dist_m > 2.0:
                ctrl.throttle = 0.20
                ctrl.brake    = 0.0
            # Hard speed cap: never exceed _PED_SPEED * 1.8.
            if cur_spd > _PED_SPEED * 1.8:
                ctrl.throttle = 0.0
                ctrl.brake    = 0.2
            vehicle.apply_control(ctrl)

        # ── pedestrian: apply WalkerControl every step ───────────────────────────
        # Pedestrian direction: always walk in vehicle's forward direction.
        # If ped drifted > 8 m, redirect back toward car until within 6.5 m.
        # No teleport — the pedestrian always moves naturally on foot.
        try:
            ped_loc     = ped.get_location()
            ego_loc2    = vehicle.get_location()
            dist_to_ego = ped_loc.distance(ego_loc2)
            if dist_to_ego > 8.0:
                dx   = ego_loc2.x - ped_loc.x
                dy   = ego_loc2.y - ped_loc.y
                norm = math.sqrt(dx**2 + dy**2 + 1e-9)
                _ped_dir = carla.Vector3D(x=dx/norm, y=dy/norm, z=0.0)
            else:
                _ped_dir = _ped_forward_dir()   # re-read every tick so turns are tracked
            _apply_walker_control(_ped_dir, _PED_SPEED)
        except Exception:
            pass
